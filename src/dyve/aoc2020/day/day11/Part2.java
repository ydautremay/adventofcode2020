...
public class Part2 {
    ...
    private boolean isNowOccupied(int i, int j) {
        for(Direction dir : Direction.values()){
            Character c = firstSeenSeat(i, j, dir);
            if(c != null && c == '#'){
                return false;
            }
        }
    ...
    private Character firstSeenSeat(int i, int j, Direction direction){
        Character c = '.';
        while (c != null && c == '.'){
            i = direction.moveHorizontally(i);
            j = direction.moveVertically(j);
            c = seats.get(i, j);
        }
        return c;
    }
